...
def NRTL_method(alpha, list_A, list_fraction, Temperature, R):
    ...
    for i in range(len(fractions)):
        ...
        for j in range(len(fractions[i])):
            if fractions[i][j] != 0:
                g_mist_aux += fractions[i][j] * np.log(fractions[i][j])
            g_mist_aux += np.log(gammas_NRTL[i][j]) * fractions[i][j]
...
fractions = list_fractions(num_points=80)
...
comp_2 = Comp(name="MIC", x=x_2, list_group=[(1, 2), (2, 1), (3, 1), (18, 1)])
